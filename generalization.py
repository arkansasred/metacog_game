...
question = visual.TextStim(win, text="Shoot or Capture?", color='Black', pos=(0,8), wrapWidth=20)
answer = visual.TextStim(win, text="1								2", color='Black', pos=(0,-2), wrapWidth=20)
reminder = visual.TextStim(win,text="  Shoot 							  Capture", color='Black', pos=(0,0),wrapWidth=20) 
...
enemyA = [visual.ImageStim(win, A, pos=(0,0), size=(80,80), units="pix", name=A[-7:-4]) for A in enemyA]
...
enemyB = [visual.ImageStim(win, B, pos=(0,0), size=(80,80), units="pix", name=B[-7:-4]) for B in enemyB]
...
for theType, response, rating in izip_longest(alienTypes, alienResponses, confidenceRatings):
	familiarityRankings.write(theType+','+response+','+rating+'\n')
