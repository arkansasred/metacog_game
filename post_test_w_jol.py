...
class Game(object):
    ...
    enemyAHitPlayerTime = []
    enemyBHitPlayerTime = []
    #lists to record prospective assesment values
    answer1Val= []
    answer2Val= []
    score = 0
    ...
    def __init__(self):
        ...
        self.q1=eztext.Input(x=SCREEN_WIDTH//2-400, y=SCREEN_HEIGHT//2, maxlength=45, color=GREEN, prompt='What percentage of Aliens do you expect to correctly shoot or capture?:  ')
        ...
        #explosion sound
        self.explosionSound = pyo.SfPlayer("Sounds/explosion.wav", speed=[1,1])
    ...
    def run_logic(self):
        ...
        # Create the enemy sprites
            ...
            if len(self.enemies_list)<(self.numberEnemies*16-1) and len(self.enemies_list)%20==0 and self.elapsedTime==0:
                self.getMetacogEval=True
    ...
    def display_frame(self, screen):
        ...
        if self.game_over:  
            font = pygame.font.Font(None, 25)
            text2 = font.render("You successfully killed "+ str((len(self.enemyAKillTime)+len(self.enemyBKillTime))/(self.numberEnemies*16)) +
                                "percent of the enemies, for a score of {:.0f}".format(self.score),
                                True, GREEN)
        ...
        if not self.game_over and not self.game_start and not self.getMetacogEval:
